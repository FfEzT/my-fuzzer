...
func main() {
  ...
  // UPD: через 2 дня понял, что это не совсем каноничное решение, (например, каналы не закрываются там, где были созданы)(или горутина А должна была запускать воркеры, а не main).
  // Однако для правильного решения потребовались бы еще механизмы синхронизации, что усложнило бы код, поэтому оставлю так, как есть
  wg := sync.WaitGroup{}
  ...
  // NOTE думаю не критично, если все потоки резко завершатся (например, для сервера было бы важно закрыть соединение с бд)
