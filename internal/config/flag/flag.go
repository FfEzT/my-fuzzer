...
func GetConfig() (result *Config, err error) {
  target      := flag.String("u", "", "host target")
  worlistPath := flag.String("w", "", "Path to wordlist")
  payload     := flag.String("d", "", "payload")

  workerCount := flag.Int("c", 3, "Count of workers")
  ...
  lines       := flag.String("fl", "", "Filter by amount of lines in response. Comma separated list of line counts and ranges")
